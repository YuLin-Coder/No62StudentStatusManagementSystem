...
layui.define(["element", "laytpl", "jquery"], function (exports) {
    ...
    var miniMenu = {
        ...
        render: function (options) {
            ...
            options.loginType = options.loginType || 1;
            options.multiModule = options.multiModule || false;
            options.menuChildOpen = options.menuChildOpen || false;
            if (options.multiModule) {
                miniMenu.renderMultiModule(options.menuList, options.menuChildOpen);
            } else {
                miniMenu.renderSingleModule(options.menuList, options.menuChildOpen, options.loginType);
            }
            miniMenu.listen();
        },
         ...
         */
        renderSingleModule: function (menuList, menuChildOpen, loginType) {
            menuList = menuList || [];
            var leftMenuHtml = '',
                childOpenClass = '',
            ...
            leftMenuHtml = this.renderLeftMenu(menuList, {childOpenClass: childOpenClass}, loginType);
        ...
        renderLeftMenu: function (leftMenus, options, loginType) {
            options = options || {};
            var me = this;
            var leftMenusHtml = me.each(leftMenus || [], function (idx, leftMenu) { // 左侧菜单遍历
                if (loginType == leftMenu.type) {
